    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/sections/terms_of_use?lang=${lang}`
        );
        setContent(response.data.content);
      } catch (err) {
        setError("فشل في تحميل شروط الاستخدام");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="max-w-5xl mx-auto px-6 py-16 text-gray-800">
      <h1 className="text-4xl font-bold mb-6 text-blue-600">
        {content.terms_of_use_title}
      </h1>

      <p className="mb-4">{parse(content.terms_of_use_intro)}</p>
        ...
        return (
          <div key={section}>
            <h2 className="text-2xl font-semibold mt-8 mb-2 text-blue-500">
              {content[`terms_of_use_section_${section}_title`]}
            </h2>
            <div className="mb-4">
              {parse(content[`terms_of_use_section_${section}_content`])}
            </div>
          </div>
        );
      })}
      ...
      <p className="text-sm text-gray-500 mt-8 text-center">
        {content.terms_of_use_last_updated}
      </p>
    </section>
  );
};
